...
private[finagle] object Bufs {
   ...
   */
  implicit def seqOfNonEmptyStringToBuf(strings: Iterable[String]): Seq[Buf] = {
    if (strings == null) {
      null
    } else {
      strings.iterator.map(nonEmptyStringToBuf).to(scala.collection.immutable.IndexedSeq)
    }
  }
